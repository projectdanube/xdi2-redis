...
public class XDIRedis extends HttpServlet implements HttpRequestHandler {

	private static final long serialVersionUID = 6222742990137314344L;

	private static final Logger log = LoggerFactory.getLogger(XDIRedis.class);
	...
	static {
		...
		while (true) {
			...
			InputStream inputStream = XDIRedis.class.getResourceAsStream("message" + (sampleInputs.size() + 1) + ".xdi");
